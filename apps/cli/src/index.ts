...
program
	.name("alpha")
	.description("Alpha CLI - starts an interactive session by default, use -p/--print for non-interactive output")
	.version(VERSION)
...
const applyListOptions = (command: Command) =>
	command
		.option("-w, --workspace <path>", "Workspace directory path (defaults to current working directory)")
		.option("-e, --extension <path>", "Path to the extension bundle directory")
		.option("-k, --api-key <key>", "Alpha API key (falls back to saved login/session token)")
...
applyListOptions(listCommand.command("models").description("List available Alpha models")).action(
