...
const RELEASES_URL = "https://api.github.com/repos/AlphaInc/Alpha/releases?per_page=100"
...
export function runUpgradeInstaller(version?: string, spawnImpl: typeof spawn = spawn): Promise<void> {
	return new Promise((resolve, reject) => {
		const env = version ? { ...process.env, ALPHA_VERSION: version } : process.env
...
export async function upgrade(options: UpgradeOptions = {}): Promise<void> {
	...
	if (compareVersions(latestVersion, currentVersion) <= 0) {
		console.log("Alpha CLI is already up to date.")
		return
	}

	console.log(`Upgrading Alpha CLI from ${currentVersion} to ${latestVersion}...`)
	if (runInstaller) {
		await runInstaller()
	} else {
