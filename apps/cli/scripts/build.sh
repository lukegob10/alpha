...
create_tarball() {
    ...
    RELEASE_DIR="$REPO_ROOT/alpha-cli-${PLATFORM}"
    ...
    cat > "$RELEASE_DIR/bin/alpha" << 'WRAPPER_EOF'
#!/usr/bin/env node
...
process.env.ALPHA_CLI_ROOT = join(__dirname, '..');
process.env.ALPHA_EXTENSION_PATH = join(__dirname, '..', 'extension');
const ripgrepPath = join(__dirname, 'rg');
if (existsSync(ripgrepPath)) {
  process.env.ALPHA_RIPGREP_PATH = ripgrepPath;
}
...
verify_local_install() {
    ...
    ALPHA_LOCAL_TARBALL="$TARBALL_PATH" \
    ALPHA_INSTALL_DIR="$VERIFY_INSTALL_DIR" \
    ALPHA_BIN_DIR="$VERIFY_BIN_DIR" \
    ...
    # Test --help
    if ! "$VERIFY_BIN_DIR/alpha" --help > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
    ...
    # Test --version
    if ! "$VERIFY_BIN_DIR/alpha" --version > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
...
print_summary() {
    ...
    if [ "$LOCAL_INSTALL" = true ]; then
        echo "  Installed to: ~/.alpha/cli"
        echo "  Binary: ~/.local/bin/alpha"
        echo ""
        echo "  Test it out:"
        echo "    alpha --version"
        echo "    alpha --help"
        ...
        echo "    ALPHA_LOCAL_TARBALL=$REPO_ROOT/$TARBALL ./apps/cli/install.sh"
        echo ""
