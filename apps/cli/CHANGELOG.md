...
- **Alpha Model Warmup**: When configured with the Alpha provider, the CLI now proactively fetches and warms the model list during activation so that model information is available before the first prompt is sent. The warmup has a 10s timeout and failures are logged only in debug mode.
...
- **Alpha Provider API Key Support**: Allow `--api-key` flag and `ROO_API_KEY` environment variable for the roo provider instead of requiring cloud auth token
...
- **Debug Logging**: Debug log file (`~/.alpha/cli-debug.log`) is now disabled by default unless `--debug` flag is passed
...
- Improved Alpha Router authentication flow and error messaging

### Fixed

- Removed unnecessary timeout that could cause issues with long-running tasks
- Fixed authentication token validation for Alpha Router provider
