...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@alpha-code/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"alpha-code-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"Alpha-Nightly"',
