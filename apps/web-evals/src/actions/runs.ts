...
export async function createRun({
	...
	try {
		...
		const cliCommand = `pnpm --filter @alpha-code/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/alpha-code-evals.log", { flags: "a" })
...
export async function deleteOldRuns(): Promise<DeleteIncompleteRunsResult> {
	...
	const { getRuns } = await import("@alpha-code/evals")
