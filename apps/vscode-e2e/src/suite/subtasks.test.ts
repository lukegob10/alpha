...
suite.skip("Alpha Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, AlphaMessage[]> = {}

		api.on(AlphaCodeEventName.Message, ({ taskId, message }) => {
		...
		api.on(AlphaCodeEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
