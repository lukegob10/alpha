...
- Poe, Alpha içinde Poe modellerine doğrudan erişebilmen için bir AI sağlayıcısı olarak eklendi.
...
Alpha, sizin çalışma şeklinize uyum sağlar, tam tersi değil:
...
- **[Dokümantasyon](https://docs.roocode.com):** Alpha'u yükleme, yapılandırma ve ustalaşma konusundaki resmi kılavuz.
...
- **[GitHub Sorunları](https://github.com/AlphaInc/Alpha/issues):** Hataları bildirin ve gelişimi takip edin.
- **[Özellik İstekleri](https://github.com/AlphaInc/Alpha/discussions/categories/feature-requests?discussions_q=is%3Aopen+category%3A%22Feature+Requests%22+sort%3Atop):** Bir fikriniz mi var? Geliştiricilerle paylaşın.
...
Alpha uzantısını çalıştırmanın birkaç yolu vardır:

### Geliştirme Modu (F5)

Aktif geliştirme için VSCode'un yerleşik hata ayıklama özelliğini kullanın:

VSCode'da `F5` tuşuna basın (veya **Çalıştır** → **Hata Ayıklamayı Başlat**'a gidin). Bu, Alpha uzantısının çalıştığı yeni bir VSCode penceresi açacaktır.
...
1.  İlk olarak, VSIX paketini oluşturun:
    ...
    pnpm vsix
    ```
2.  `bin/` dizininde bir `.vsix` dosyası oluşturulur (ör. `bin/alpha-<version>.vsix`).
...
**Alpha'un keyfini çıkar!** Onu kısa tasmalı tut ya da otonom dolaşsın; ne inşa edeceğini görmek için sabırsızlanıyoruz. Soruların veya özellik fikirlerin varsa [Reddit topluluğumuza](https://www.reddit.com/r/RooCode/) ya da [Discord](https://discord.gg/roocode)'a uğra. İyi kodlamalar!
