...
- Aggiunto Poe come provider AI così puoi accedere direttamente ai modelli Poe in Alpha.
...
Alpha si adatta al tuo modo di lavorare, non il contrario:
...
- **[Documentazione](https://docs.roocode.com):** La guida ufficiale per installare, configurare e padroneggiare Alpha.
...
- **[Problemi GitHub](https://github.com/AlphaInc/Alpha/issues):** Segnala bug e tieni traccia dello sviluppo.
- **[Richieste di funzionalità](https://github.com/AlphaInc/Alpha/discussions/categories/feature-requests?discussions_q=is%3Aopen+category%3A%22Feature+Requests%22+sort%3Atop):** Hai un'idea? Condividila con gli sviluppatori.
...
Ci sono diversi modi per eseguire l'estensione Alpha:

### Modalità di sviluppo (F5)

Per lo sviluppo attivo, usa il debug integrato di VSCode:

Premi `F5` (o vai su **Esegui** → **Avvia debug**) in VSCode. Si aprirà una nuova finestra di VSCode con l'estensione Alpha in esecuzione.
...
1.  Per prima cosa, compila il pacchetto VSIX:
    ...
    pnpm vsix
    ```
2.  Un file `.vsix` verrà generato nella directory `bin/` (ad es. `bin/alpha-<version>.vsix`).
