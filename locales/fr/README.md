...
- Ajout de Poe comme fournisseur d’IA pour que tu puisses accéder directement aux modèles Poe dans Alpha.
...
## Que peut faire Alpha pour VOUS ?
...
Alpha s'adapte à votre façon de travailler, pas l'inverse :
...
- **[Documentation](https://docs.roocode.com):** Le guide officiel pour installer, configurer et maîtriser Alpha.
...
- **[Problèmes GitHub](https://github.com/AlphaInc/Alpha/issues):** Signalez des bugs et suivez le développement.
- **[Demandes de fonctionnalités](https://github.com/AlphaInc/Alpha/discussions/categories/feature-requests?discussions_q=is%3Aopen+category%3A%22Feature+Requests%22+sort%3Atop):** Vous avez une idée ? Partagez-la avec les développeurs.
...
Il existe plusieurs façons d'exécuter l'extension Alpha :

### Mode développement (F5)

Pour un développement actif, utilisez le débogage intégré de VSCode :

Appuyez sur `F5` (ou allez dans **Exécuter** → **Démarrer le débogage**) dans VSCode. Cela ouvrira une nouvelle fenêtre VSCode avec l'extension Alpha en cours d'exécution.
...
1.  D'abord, compilez le package VSIX :
    ...
    pnpm vsix
    ```
2.  Un fichier `.vsix` sera généré dans le répertoire `bin/` (par exemple, `bin/alpha-<version>.vsix`).
