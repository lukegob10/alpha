...
without addressing these gaps. Several useful changes are small. Read output cursors landed 16 September 2026; fully
competitive command-session control and streaming large-file I/O still need more deliberate work.
...
**Status: output-cursor mismatch fixed 16 September 2026; whole-file slurp remains.**

The original failure: [ReadFileTool.ts](../src/core/tools/ReadFileTool.ts) formatted up to 2,000 numbered lines, then
[ToolScheduler.ts](../src/core/agent/ToolScheduler.ts) clipped at 32,000 characters. Continuation pointed at line 2001
(or vanished with `[Tool output truncated by harness]`) while the model had only seen a few hundred lines.

**Implementation follow-up (September 16, 2026):** `readFileContent.ts` selects ranges, then renders into the scheduler's
remaining text allowance (`getRemainingOutputChars`, approval text already subtracted). Pages stop on complete lines.
Partial results carry a path- and content-hash-bound `continuation` for the first unread line or column. Batches share
one allowance. Indentation is a selector, not a syntactic guarantee; oversized selections paginate instead of claiming
the whole block was delivered. Invalid indentation anchors error. Saved string/tuple ranges no longer override the
parser's canonical ranges. Investigation guidance was tightened in the tool-use section; it does not change budgets or
authority.

Deterministic pagination tests cover the real scheduler with approval feedback, 32k resume without a hole, batch
sharing, Unicode fragments, stale/cross-file cursors, and disjoint ranges. Live Copilot Luna on VS Code 1.136.1 paged
1–501 then 502–1,000 inside the cap. Details: [read evidence tests](read-evidence-tests.md).

**Still open:** ordinary text still slurps the file before slicing (`fs.readFile`). `@` mentions still advertise
`offset`/`limit=2000`. Streaming I/O and language navigation were not part of this change.
...
**Implementation follow-up (September 15, 2026):** `edit`, `search_replace`, `edit_file`, and `apply_patch` updates now
share LF matching with restoration of the original LF/CRLF convention and BOM. Mixed LF/CRLF files are rejected before
approval rather than silently converted. Patch updates retain the absence of a final newline; an explicitly added empty
line can supply one. Native `apply_diff` arguments are literal for every provider. The native-only execution boundary
already rejects legacy XML calls, so no textual decoder was added. Command decoding and `write_to_file` remain follow-ups.

`apply_patch` preflights each operation sequentially without writing, including access, protection, existence, matching,
and move-destination baselines. Valid independent operations can proceed when another fails preflight. Runtime failure,
denial, or cancellation stops later writes. Its JSON `files` ledger lists each operation's path, optional move path,
`applied`/`skipped`/`error` status, and result or reason. Earlier successful writes remain committed. The scheduler retains
this ledger alongside approval feedback and preserves settled mutation details on cancellation. Existing diff previews,
checkpoints, mutation gates, and stale-content checks still own writes; this is not an atomic multi-file transaction.
...
| `read_file`                                                                                                                                                                                          | Budget-aware pages, honest continuation, shared batch allowance, version-bound cursors. Indentation no longer claims syntactic completeness. Whole-file slurp remains. | Bounded streaming/sliced I/O for large text; align `@` mention continuation with the tool cursor. Language navigation is a later capability.                                                                 |
...
| 6     | Align read output budgets/cursors and add bounded large-file slices.                           | Medium       | **Cursors/budgets landed 16 Sep 2026** (see read-evidence-tests.md). Streaming slices and memory measurements remain. |
...
| Flagged for follow-up       | TODO isolation; browser approval workaround; GitHub approval/cancellation; native entity decoding; read **streaming/sliced I/O** (output cursors landed 16 Sep 2026); newline consistency (edit-byte restore landed 15 Sep); command control and search ergonomics (search modes landed 15 Sep). |
