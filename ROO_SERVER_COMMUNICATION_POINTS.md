# Roo/Alpha Server Communication Points

This document inventories code paths in this repo that communicate with Roo/Alpha-operated services or account/cloud infrastructure. It focuses on the VS Code extension and adjacent CLI paths, not generic third-party model providers such as Anthropic, OpenAI, Gemini, Ollama, OpenRouter, Requesty, etc.

Default Roo/Alpha service hosts are defined in `packages/cloud/src/config.ts`:
...
| Roo model proxy | `https://api.roocode.com/proxy` | `ROO_CODE_PROVIDER_URL` |
...
- `src/extension.ts`
  ...
  - refreshes Roo model cache on cloud auth state changes
...
- Roo provider settings can prompt cloud connection.
...
2. The `@roo-code/cloud` package or its imported usage from extension code.
...
- Documentation, GitHub, YouTube, Discord, Reddit, Substack, marketplace.visualstudio.com links in website/docs.
