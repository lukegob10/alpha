...
function SelectTrigger({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Trigger>) {
	return (
		<SelectPrimitive.Trigger
			...
			className={cn(
				"data-[placeholder]:text-muted-foreground [&_svg:not([class*='text-'])]:text-muted-foreground aria-invalid:border-destructive flex h-8 w-fit items-center justify-between gap-2 rounded-lg px-3 py-2 whitespace-nowrap transition-[color,border-color,background-color,box-shadow] duration-150 outline-none disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 cursor-pointer",
				"border border-[var(--border-subtle)] aria-expanded:border-[var(--border-accent)] focus-visible:border-[var(--border-accent)]",
				"bg-[var(--surface-sunken)] hover:border-[var(--border-accent)] hover:bg-[var(--alpha-accent-soft)] focus-visible:ring-1 focus-visible:ring-vscode-focusBorder",
