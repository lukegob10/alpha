...
describe("Announcement", () => {
	it("renders the current release announcement", () => {
		...
		expect(screen.getByText("Welcome to Alpha v2.1.48")).toBeInTheDocument()
		expect(
			screen.getByText("Alpha v2.1.48 improves verification, command control, and task continuity."),
		).toBeInTheDocument()
	})
	...
	it("uses localized release text with the current package version", () => {
		...
		expect(
			screen.getByText(
				"Alpha v2.1.48 verbessert die Überprüfung, die Befehlssteuerung und die Fortführung von Aufgaben.",
			),
		).toBeInTheDocument()
		expect(
			screen.getByText(
