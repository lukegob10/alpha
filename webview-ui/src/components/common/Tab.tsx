...
export const Tab = ({ className, children, ...props }: TabProps) => (
	<div className={cn("app-shell fixed inset-0 flex flex-col", className)} {...props}>
		{children}
	</div>
)

export const TabHeader = ({ className, children, ...props }: TabProps) => (
	<div className={cn("glass-header z-10 border-b px-5 py-3", className)} {...props}>
		{children}
	</div>
)
...
>(({ children, className, value: _value, isSelected, onSelect, ...props }, ref) => {
	return (
		<button
			...
			className={cn(
				"rounded-lg transition-[color,background-color,box-shadow] duration-150 focus:outline-none focus-visible:ring-1 focus-visible:ring-vscode-focusBorder",
				className,
			)}
			onClick={onSelect}
			{...props}>
