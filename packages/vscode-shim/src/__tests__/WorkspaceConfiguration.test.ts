...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("alpha", context)
			...
			setRuntimeConfig("alpha", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("alpha", "testSetting", "testValue")

			expect(getRuntimeConfig("alpha.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("alpha", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("alpha.setting1")).toBe("value1")
			expect(getRuntimeConfig("alpha.setting2")).toBe(42)
			expect(getRuntimeConfig("alpha.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("alpha.defined")).toBe("value")
			expect(getRuntimeConfig("alpha.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("alpha", "setting1", "value1")
			setRuntimeConfig("alpha", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("alpha.setting1")).toBeUndefined()
			expect(getRuntimeConfig("alpha.setting2")).toBeUndefined()
		})
