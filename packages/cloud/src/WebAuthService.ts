...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			const publisher = packageJSON?.publisher ?? "Alpha"
			const name = packageJSON?.name ?? "alpha"
			...
			this.log(`[auth] Error initiating Alpha Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Alpha Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Alpha Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Alpha Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Alpha Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Alpha Cloud")
			}

			this.log("[auth] Successfully authenticated with Alpha Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Alpha Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Alpha Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Alpha Cloud")
			}

			this.log("[auth] Logged out from Alpha Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Alpha Cloud: ${error}`)
			throw new Error(`Failed to log out from Alpha Cloud: ${error}`)
		}
	}
