...
describe("WebAuthService", () => {
	...
	beforeEach(() => {
		...
		// Setup utils mock
		vi.mocked(getUserAgent).mockReturnValue("Alpha 1.0.0")
	...
	describe("login", () => {
		...
		it("should use package.json values for redirect URI with default sign-in endpoint", async () => {
			...
			const expectedUrl =
				"https://api.test.com/extension/sign-in?state=746573742d72616e646f6d2d6279746573&auth_redirect=vscode%3A%2F%2FAlpha.alpha"
			expect(mockOpenExternal).toHaveBeenCalledWith(
				expect.objectContaining({
					toString: expect.any(Function),
				}),
		...
		it("should use provider signup URL when useProviderSignup is true", async () => {
			...
			const expectedUrl =
				"https://api.test.com/extension/provider-sign-up?state=746573742d72616e646f6d2d6279746573&auth_redirect=vscode%3A%2F%2FAlpha.alpha"
			expect(mockOpenExternal).toHaveBeenCalledWith(
				expect.objectContaining({
					toString: expect.any(Function),
				}),
		...
		it("should handle errors during login", async () => {
			...
			await expect(authService.login()).rejects.toThrow("Failed to initiate Alpha Cloud authentication")
			expect(mockLog).toHaveBeenCalledWith("[auth] Error initiating Alpha Cloud auth: Error: Crypto error")
		})
	})
	...
	describe("handleCallback", () => {
		...
		it("should handle invalid parameters", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Invalid Alpha Cloud sign in url")
		...
		it("should validate state parameter", async () => {
			...
			await expect(authService.handleCallback("code", "different-state")).rejects.toThrow(
				"Failed to handle Alpha Cloud callback",
			)
		...
		it("should successfully handle valid callback", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Successfully authenticated with Alpha Cloud")
		})
	...
	describe("logout", () => {
		...
		it("should clear credentials and call Clerk logout", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Logged out from Alpha Cloud")
		})
