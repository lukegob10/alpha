...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "Alpha",
	userEmail = "support@alpha.invalid",
	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "Alpha")
				await mainGit.addConfig("user.email", "support@alpha.invalid")
				...
				await nestedGit.addConfig("user.name", "Alpha")
				await nestedGit.addConfig("user.email", "support@alpha.invalid")
		...
		describe(`${klass.name}#saveCheckpoint with allowEmpty option`, () => {
			...
			it("isolates checkpoint operations from GIT_DIR environment variable", async () => {
				...
				// In the real-world Dev Container scenario, GIT_DIR is set BEFORE Alpha starts,
