...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
			...
			void provider.providerSettingsManager
				...
				.then(async (listApiConfig) => {
					...
					if (listApiConfig.length === 1) {
						// Check if first time init then sync with the current configuration when
						// the persisted profile has no model metadata yet.
						if (!listApiConfig[0].modelId) {
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the Alpha commands
