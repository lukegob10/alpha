// npx vitest core/webview/__tests__/AlphaProvider.sticky-mode.spec.ts
...
describe("AlphaProvider - Sticky Mode", () => {
	...
	describe("Visible lane state", () => {
		...
		it("creates delegated child tasks with the provider profile mapped to the child mode", async () => {
			...
			await provider.addTaskToStack(parentTask as any)
	...
	describe("New task mode boundary", () => {
		...
		it("defaults an immediate top-level task created from a Plan task to Code", async () => {
			...
			await provider.addTaskToStack(planTask as any)
			await provider.setValue("mode", "architect")
		...
		it("honors an explicit Plan selection made after opening a blank draft", async () => {
			...
			await provider.addTaskToStack(previousTask as any)
			await provider.startBlankTask()
	...
	describe("handleModeSwitch", () => {
		...
		it("should update task's taskMode property when switching modes", async () => {
			...
			// Add task to provider stack
			await provider.addTaskToStack(mockTask as any)
	...
	describe("Integration with new_task tool", () => {
		it("should preserve parent task mode when creating subtasks", async () => {
			...
			await provider.addTaskToStack(subtask)
	...
	describe("Concurrent mode switches", () => {
		...
		it("should handle mode switches during task save operations", async () => {
			...
			const mockTask = {
				...
				_taskMode: "code",
				emit: vi.fn(),
				saveAlphaMessages: vi.fn().mockImplementation(async () => {
					// Simulate slow save
			...
			const savePromise = mockTask.saveAlphaMessages()
		...
		it("should handle errors during mode switch gracefully", async () => {
			...
			const mockTask = {
				...
				emit: vi.fn().mockImplementation((event) => {
					...
					// The first call is for TaskFocused in addTaskToStack
	...
	describe("Multiple tasks switching modes simultaneously", () => {
		it("should handle multiple tasks switching modes independently", async () => {
			...
			await provider.addTaskToStack(task1 as any)
			await provider.addTaskToStack(task2 as any)
			await provider.addTaskToStack(task3 as any)
