...
export async function handleListWorktrees(provider: AlphaProvider): Promise<WorktreeListResponse> {
...
export async function handleSwitchWorktree(
	...
	try {
		...
		if (newWindow) {
			// Set the auto-open path so the new window opens Alpha sidebar.
...
export async function handleGetAvailableBranches(provider: AlphaProvider): Promise<BranchInfo> {
	const cwd = provider.cwd
...
export async function handleGetWorktreeDefaults(provider: AlphaProvider): Promise<WorktreeDefaultsResponse> {
	const suffix = generateRandomSuffix()
	const workspaceFolders = vscode.workspace.workspaceFolders
	const projectName = workspaceFolders?.[0]?.name || "project"

	const legacyConfigPath = path.join(os.homedir(), ".roo")
	const suggestedPath = path.join(legacyConfigPath, "worktrees", `${projectName}-${suffix}`)

	return {
		suggestedBranch: `worktree/alpha-${suffix}`,
		suggestedPath,
	}
}

export async function handleGetWorktreeIncludeStatus(provider: AlphaProvider): Promise<WorktreeIncludeStatus> {
	const cwd = provider.cwd
	return worktreeIncludeService.getStatus(cwd)
}

export async function handleCheckBranchWorktreeInclude(provider: AlphaProvider, branch: string): Promise<boolean> {
	const cwd = provider.cwd
	return worktreeIncludeService.branchHasWorktreeInclude(cwd, branch)
}

export async function handleCreateWorktreeInclude(provider: AlphaProvider, content: string): Promise<WorktreeResult> {
	const cwd = provider.cwd
...
export async function handleCheckoutBranch(provider: AlphaProvider, branch: string): Promise<WorktreeResult> {
	const cwd = provider.cwd
