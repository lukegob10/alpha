...
describe("ReadCommandOutputTool", () => {
	...
	describe("Search filtering", () => {
		...
		it("should report the bounded search range in tool telemetry", async () => {
			...
			await tool.execute(
				{ artifact_id: artifactId, search: "^$", limit: READ_COMMAND_OUTPUT_MIN_LIMIT_BYTES },
				mockTask,
				mockCallbacks,
			)
		...
		it.each(["a+a+$", "(a|aa)+$", "(a?)+$", "(?:a{1,2})+$", "a{0,1000}a{1000}b"])(
		...
		])("should allow a flat quantifier with a suffix and return matching lines: %s", async (search, content) => {
			const artifactId = "cmd-1706119234567.txt"
			setupSearchMock(content)
		...
		it.each([64, 1000])(
			"should allow a large fixed repetition when it is the terminal token: %s",
			async (count) => {
				const artifactId = "cmd-1706119234567.txt"
				setupSearchMock(`${"a".repeat(count)}\n`)

				await tool.execute(
					{ artifact_id: artifactId, search: `a{${count}}`, limit: 2048 },
					mockTask,
					mockCallbacks,
				)
	...
	describe("Error handling", () => {
		...
		it("should report access failures as errors without calling them missing artifacts", async () => {
			...
			expect(mockCallbacks.setResultMetadata).toHaveBeenCalledWith({ status: "error" })
			expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(
				expect.stringContaining("Unable to access artifact"),
			)
		...
		it.each([
			...
			expect(mockCallbacks.setResultMetadata).toHaveBeenCalledWith({ status: "error" })
			expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(
				expect.stringContaining("Error reading command output"),
			)
		})
		...
		it("should preserve cancellation and close a handle when a read is pending", async () => {
			...
			const execution = tool.execute({ artifact_id: artifactId }, mockTask, {
				...mockCallbacks,
