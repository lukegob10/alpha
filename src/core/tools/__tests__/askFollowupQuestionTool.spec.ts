...
describe("askFollowupQuestionTool", () => {
	...
	it("should parse suggestions without mode attributes", async () => {
		...
		await askFollowupQuestionTool.handle(mockAlphaTask, block as ToolUse<"ask_followup_question">, {
			askApproval: vi.fn(),
			handleError: vi.fn(),
			pushToolResult: mockPushToolResult,
		})

		expect(mockAlphaTask.ask).toHaveBeenCalledWith(
			"followup",
	...
	describe("parameter validation", () => {
		it("should handle missing follow_up parameter", async () => {
			...
			expect(mockAlphaTask.sayAndCreateMissingParamError).toHaveBeenCalledWith(
				"ask_followup_question",
				"follow_up",
			)
			expect(mockAlphaTask.recordToolError).toHaveBeenCalledWith("ask_followup_question")
			expect(mockAlphaTask.didToolFailInCurrentTurn).toBe(true)
	...
	describe("handlePartial with native protocol", () => {
		it("should only send question during partial streaming to avoid raw JSON display", async () => {
			...
			await askFollowupQuestionTool.handle(mockAlphaTask, block, {
				askApproval: vi.fn(),
				handleError: vi.fn(),
				pushToolResult: mockPushToolResult,
			})
			...
			expect(mockAlphaTask.ask).toHaveBeenCalledWith("followup", "What would you like to do?", true)
		})
		...
		it("should handle partial with question from params", async () => {
			...
			expect(mockAlphaTask.ask).toHaveBeenCalledWith("followup", "Choose wisely", true)
		})
	})
