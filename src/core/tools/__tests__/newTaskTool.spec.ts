...
type MockAlphaInstance = { taskId: string }
...
const mockStartSubtask = vi
	.fn<(message: string, todoItems: any[], mode: string) => Promise<MockAlphaInstance>>()
...
// Mock the Alpha instance and its methods/properties
const mockAlphaTask = {
	ask: vi.fn(),
...
describe("newTaskTool", () => {
	it.each(["ask", "debug", "orchestrator", "custom-mode"])(
		...
		async (mode) => {
			await newTaskTool.execute({ mode, message: "Investigate" }, mockAlphaTask as any, {
	...
	it("should correctly un-escape \\\\@ to \\@ in the message passed to the new task", async () => {
		...
		await newTaskTool.handle(mockAlphaTask as any, withNativeArgs(block), {
			askApproval: mockAskApproval,
			handleError: mockHandleError,
			pushToolResult: mockPushToolResult,
		})
	...
	it("should handle missing todos parameter gracefully (backward compatibility)", async () => {
		...
		expect(mockAlphaTask.recordToolError).not.toHaveBeenCalledWith("new_task")
	...
	it("should error when mode parameter is missing", async () => {
		...
		expect(mockAlphaTask.recordToolError).toHaveBeenCalledWith("new_task")
	})
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("alpha")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (alpha-code-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "alpha-code-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("alpha-code-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
...
describe("newTaskTool delegation flow", () => {
	it("delegates to provider and does not call legacy startSubtask", async () => {
		...
		// Act
		await newTaskTool.handle(localAlpha as any, withNativeArgs(block), {
			askApproval: mockAskApproval,
			handleError: mockHandleError,
			pushToolResult: mockPushToolResult,
		})
