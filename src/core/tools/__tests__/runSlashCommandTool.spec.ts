...
Use skill workflow`,
	...
	it("should preserve command precedence over skill fallback", async () => {
		...
		const mockCommand = {
			...
			source: "project" as const,
			filePath: ".alpha/commands/setup.md",
		...
		const getSkillContent = vi.fn().mockResolvedValue({
			...
			path: "/mock/.alpha/skills/setup/SKILL.md",
			source: "project" as const,
...
Initialize project content here`,
	...
	it("should successfully execute command with arguments", async () => {
		...
		const mockCommand = {
			...
			source: "project" as const,
			filePath: ".alpha/commands/test.md",
...
Run tests with specific focus`,
	...
	it("should handle global command", async () => {
		...
		const mockCommand = {
			...
			filePath: "~/.alpha/commands/deploy.md",
		}
...
Deploy application to production`,
	...
	it.each(["debug", "architect", "code"])(
		"does not switch to %s from model-invoked command metadata",
		async (mode) => {
			...
			const mockCommand = {
				...
				source: "project" as const,
				filePath: ".alpha/commands/debug-app.md",
			...
			expect(mockSetTaskMode).not.toHaveBeenCalled()
			expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(
				`Command: /debug-app
Description: Debug the application
Source: project
...
Start debugging the application`,
	...
	it("does not advertise a mode transition in the command approval message", async () => {
		const block: ToolUse<"run_slash_command"> = {
			type: "tool_use" as const,
			name: "run_slash_command" as const,
			params: {},
