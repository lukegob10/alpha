		...
		if (signal?.aborted) {
			if (fileHandle) {
				...
				void openPromise.then(
					(lateFileHandle) => closeArtifactQuietly(lateFileHandle),
					() => undefined,
				)
			}
		}
...
export class ReadCommandOutputTool extends BaseTool<"read_command_output"> {
		...
		try {
			...
			if (search !== undefined) {
				if (
					...
					search.length < 1 ||
					search.length > READ_COMMAND_OUTPUT_MAX_SEARCH_LENGTH
				) {
					await reportError(
				...
				if (isUnsafeSearchPattern(search)) {
					await reportError(
						"Invalid search pattern. Potentially unsafe regular-expression constructs are rejected.",
					)
					return
				}
			}
			...
			const taskDir = await awaitWithAbort(
				Promise.resolve(getTaskDirectoryPath(globalStoragePath, task.taskId)),
				signal,
			)
			...
			try {
				...
				const isMissingArtifact =
					errorCode === "ENOENT" || (error instanceof Error && error.message === "ENOENT")
	...
	private formatArtifactIdForError(artifactId: unknown): string {
		...
		const displayed = encodedBytes > maxBytes ? `${this.truncateUtf8(value, maxBytes - 3)}...` : value
	...
	private async searchInArtifact(
		...
		const retainMatch = (matchedLineNumber: number, line: string) => {
			...
			const retainedLineBytes =
				separatorBytes + Buffer.byteLength(linePrefix, "utf8") + Buffer.byteLength(retainedContent, "utf8")
		...
		try {
			...
			if (!truncated) {
				...
				processedBytes = Math.max(
					processedBytes,
					processDecodedChunk(decoded, loadedBytes, decoderPendingBytes),
				)
		...
		let content =
			...
			(matchedLines
				...
				: observedMatchCount === 0
					? ""
					: "No matching lines fit within the requested output limit.")
