...
describe("CustomModesManager", () => {
	...
	const mockAlphaModes = path.join(mockWorkspacePath, ".alphamodes")
	...
	beforeEach(() => {
		...
		;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
			return path === mockSettingsPath || path === mockAlphaModes
		})
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .alphamodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const projectModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: settingsModes })
				}
				if (path === mockAlphaModes) {
		...
		it("should merge modes with .alphamodes taking precedence", async () => {
			const settingsModes = [
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from alphamodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .alphamodes since it takes precedence
		...
		it("should handle missing .alphamodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .alphamodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .alphamodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .alphamodes precedence", async () => {
			...
			let alphaModesContent = { customModes: projectModes }

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockAlphaModes) {
					return yaml.stringify(alphaModesContent)
				}
				if (path === mockSettingsPath) {
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockAlphaModes) {
					alphaModesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .alphamodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .alphamodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .alphamodes file when adding project-specific mode", async () => {
			...
			// Mock .alphamodes to not exist initially
			let alphaModesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .alphamodes was created with the project mode
			...
			const writeCall = (fs.writeFile as Mock).mock.calls[0]
			expect(path.normalize(writeCall[0])).toBe(path.normalize(mockAlphaModes))

			// Verify the content written to .alphamodes
			expect(alphaModesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				;(fs.readFile as Mock).mockImplementation(async (path: string) => {
					...
					if (path === mockAlphaModes && alphaModesContent) {
						return yaml.stringify(alphaModesContent)
					}
					throw new Error("File not found")
				})
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".alphamodes"),
			...
			it("should import multiple modes at once", async () => {
				...
				expect(result.success).toBe(true)
				expect(alphaModesContent.customModes).toHaveLength(2)
				expect(alphaModesContent.customModes[0].slug).toBe("mode1")
				expect(alphaModesContent.customModes[1].slug).toBe("mode2")
			})
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .alphamodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .alphamodes file", async () => {
			const alphaModesContent = { customModes: [{ slug: "other-mode", name: "Other Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockAlphaModes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockAlphaModes) {
					return yaml.stringify(alphaModesContent)
		...
		it("should return false when .alphamodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when rules directory doesn't exist", async () => {
			const alphaModesContent = { customModes: [{ slug: "test-mode", name: "Test Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockAlphaModes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.alpha/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .alphamodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .alphamodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .alphamodes", async () => {
			const alphaModesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .alphamodes", async () => {
			const alphaModesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.alpha/rules-code" },
			])
