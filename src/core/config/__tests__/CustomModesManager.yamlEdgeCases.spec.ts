...
describe("CustomModesManager - YAML Edge Cases", () => {
	...
	const mockAlphaModes = `${path.sep}mock${path.sep}workspace${path.sep}.alphamodes`
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithNonBreakingSpaces,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithZeroWidth,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should normalize various quote characters", async () => {
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithFancyQuotes,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Complex fileRegex handling", () => {
		it("should handle complex fileRegex syntax gracefully", async () => {
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithComplexFileRegex,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("UTF-8 encoding", () => {
		...
		it("should handle various international characters", async () => {
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithInternational,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Additional edge cases", () => {
		it("should handle mixed line endings (CRLF vs LF)", async () => {
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithMixedLineEndings,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle multiple BOMs in sequence", async () => {
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithMultipleBOMs,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle deeply nested structures with edge case characters", async () => {
			...
			mockFsReadFile({
				[mockAlphaModes]: yamlWithComplexNesting,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
