...
export function buildSubagentPrompt({
	...
	return [
		...
		"Work in a bounded research cadence: use at most three research turns. Locate candidates once, then read up to eight independent files in one read_file files batch. Do not read one known file per turn. Reserve the next turn for synthesis.",
		"If a bounded locate or search establishes that named targets are absent from the current workspace, stop and synthesize immediately. Do not spend remaining turns searching unrelated hidden or support directories unless the objective identifies them as evidence sources.",
