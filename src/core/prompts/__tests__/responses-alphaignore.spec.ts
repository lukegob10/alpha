// npx vitest core/prompts/__tests__/responses-alphaignore.spec.ts
...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.alphaIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.alphaIgnoreError("secrets/api-keys.json")
			...
			expect(parsed.suggestion).toContain("update the .alphaignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.alphaIgnoreError(testPath)
				const parsed = JSON.parse(errorMessage) as any
	...
	describe("formatResponse.formatFilesList with AlphaIgnoreController", () => {
		/**
		 * Tests file listing with alphaignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new AlphaIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with AlphaIgnoreController
		 */
		it("should handle truncation with AlphaIgnoreController", async () => {
			// Create controller
			const controller = new AlphaIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with AlphaIgnoreController", async () => {
			// Create controller
			const controller = new AlphaIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .alphaignore instructions for the LLM", async () => {
			// Create controller
			const controller = new AlphaIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .alphaignore exists", async () => {
			// Set up no .alphaignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .alphaignore
			const controller = new AlphaIgnoreController(TEST_CWD)
			await controller.initialize()
