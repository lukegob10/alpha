...
describe("loadRuleFiles", () => {
	...
	it("should use .alpha/rules/ directory when it exists and has files", async () => {
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.alpha/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.alpha/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.alpha/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/file2.txt") {
	...
	it("should filter out cache files from .alpha/rules/ directory", async () => {
		// Simulate .alpha/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.alpha/rules/rule1.txt") {
				return Promise.resolve("rule 1 content")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/rule2.md") {
			...
			if (normalizedPath === "/fake/path/.alpha/rules/.DS_Store") {
				return Promise.resolve("DS_STORE_BINARY_CONTENT")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/Thumbs.db") {
				return Promise.resolve("THUMBS_DB_CONTENT")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/backup.bak") {
				return Promise.resolve("BACKUP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/cache.log") {
				return Promise.resolve("LOG_CONTENT")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/temp.tmp") {
				return Promise.resolve("TEMP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/script.pyc") {
		...
		const expectedCacheFiles = [
			"/fake/path/.alpha/rules/.DS_Store",
			"/fake/path/.alpha/rules/Thumbs.db",
			"/fake/path/.alpha/rules/backup.bak",
			"/fake/path/.alpha/rules/cache.log",
			"/fake/path/.alpha/rules/temp.tmp",
			"/fake/path/.alpha/rules/script.pyc",
		]
	...
	it("should fall back to .alpharules when .alpha/rules/ is empty", async () => {
		// Simulate .alpha/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .alpha/rules/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.alpha/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.alpha/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.alpha/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/subdir/subdir2/nested2.txt") {
...
describe("addCustomInstructions", () => {
	...
	it("should include AGENTS.md content along with other rules", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (pathStr.endsWith(".alpharules")) {
		...
		// Should contain both AGENTS.md and .alpharules content
		expect(result).toContain("# Agent Rules Standard (AGENTS.md):")
		expect(result).toContain("Agent rules content")
		expect(result).toContain("# Rules from .alpharules:")
		expect(result).toContain("Alpha rules content")
	})
	...
	it("should handle missing mode-specific rules file", async () => {
		...
		expect(result).toContain("Global Instructions:")
		expect(result).toContain("Mode-specific Instructions:")
		expect(result).not.toContain("Rules from .alpharules-test-mode")
	})
	...
	it("should use .alpha/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .alpha/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.alpha/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.alpha/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.alpha/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.alpha/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.alpha/rules-test-mode/rule2.txt") {
	...
	it("should fall back to .alpharules-test-mode when .alpha/rules-test-mode/ does not exist", async () => {
		// Simulate .alpha/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })

		// Simulate .alpharules-test-mode exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().includes(".alpharules-test-mode")) {
	...
	it("should correctly format content from directories when using .alpha/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.alpha/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.alpha/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.alpha/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.alpha/nested-symlink-target.txt") {
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.alpha/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.alpha/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.alpha/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.alpha/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.alpha/rules/symlink-target-dir/subdir_link.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.alpha/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				...
				"/fake/path/.alpha/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.alpha/rules/file3.txt") {
	...
	it("should return files in alphabetical order by filename", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "zebra.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules" },
			{ name: "alpha.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules" },
			{ name: "Beta.txt", isFile: () => true, parentPath: "/fake/path/.alpha/rules" }, // Test case-insensitive sorting
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			const pathStr = filePath.toString()
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.alpha/rules/zebra.txt") {
				return Promise.resolve("zebra content")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/alpha.txt") {
				return Promise.resolve("alpha content")
			}
			if (normalizedPath === "/fake/path/.alpha/rules/Beta.txt") {
