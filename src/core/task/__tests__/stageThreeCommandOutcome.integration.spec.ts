...
describe("Stage Three command outcome integration", () => {
	it("records the physical process outcome without deriving assurance from command output", async () => {
		await withTaskHarness(async (harness) => {
			const terminal = controlledTerminal(harness.workspacePath, (terminalProcess) => {
				execFile(
					process.execPath,
					["--version"],
					{ cwd: harness.workspacePath, windowsHide: true },
					(error, stdout, stderr) => {
						void terminalProcess.complete(
			...
			const run = createScheduler(harness, []).run(response("physical-node", "node --version"))
			await run

			expect(terminal.runCommand).toHaveBeenCalledOnce()
			expect(complete).toHaveBeenCalledExactlyOnceWith("physical-node", { exitCode: 0 }, expect.any(String))
			expect(harness.task.getCommandExecutionEvidence()[0]).toEqual(
				expect.objectContaining({ toolCallId: "physical-node", status: "succeeded", exitCode: 0 }),
			)
			expect(harness.task.getCommandExecutionEvidence()[0]).not.toHaveProperty("testValidation")
		})
	})
