...
import type { AlphaMessage, AlphaSay } from "@alpha-code/types"
...
describe("combineApiRequests", () => {
	...
	const createStartMessage = (text: string = '{"request":"GET /api/data"}', ts: number = 1000): AlphaMessage => ({
		type: "say",
		say: "api_req_started",
		text,
		ts,
	...
	const createFinishMessage = (text: string = '{"cost":0.005}', ts: number = 1001): AlphaMessage => ({
		type: "say",
	...
	const createOtherMessage = (
		...
		ts: number = 999,
	): AlphaMessage => ({ type: "say", say, text, ts })

	describe("Basic functionality", () => {
		it("should combine a pair of api_req_started and api_req_finished messages", () => {
			const messages: AlphaMessage[] = [createStartMessage(), createFinishMessage()]
		...
		it("should preserve non-API messages", () => {
			...
			const messages: AlphaMessage[] = [otherMessage, createStartMessage(), createFinishMessage()]
	...
	describe("Edge cases", () => {
		...
		it("should keep api_req_started message if no matching api_req_finished is found", () => {
			...
			const messages: AlphaMessage[] = [startMessage]
		...
		it("should handle missing text field in api_req_started", () => {
			const startMessage: AlphaMessage = {
				type: "say",
				say: "api_req_started",
				ts: 1000,
			...
			const messages: AlphaMessage[] = [startMessage, finishMessage]
		...
		it("should handle missing text field in api_req_finished", () => {
			const startMessage = createStartMessage()
			const finishMessage: AlphaMessage = {
				type: "say",
				say: "api_req_finished",
				ts: 1001,
				// text field is missing
	...
	describe("Bug verification", () => {
		it("should correctly replace api_req_started messages with combined ones", () => {
			...
			const messages: AlphaMessage[] = [otherMessage, startMessage1, finishMessage1, startMessage2]
