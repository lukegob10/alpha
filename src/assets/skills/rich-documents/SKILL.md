...
Use a relationship diagram only for a small directed acyclic flow or dependency structure; its three-column table remains the canonical evidence. Use a plain table for cyclic or unsupported graph forms. For visual evidence, use actual workspace PNG/JPEG assets with `data-image`, descriptive alt text, and a provenance caption. Respect the documented image byte/pixel limits. Images are workspace dependencies, not remote fetches or content to embed as authored base64. Do not invent screenshots or claim a schematic depicts the running product. The complete spec example demonstrates diagrams, images, and timelines; the report demonstrates metrics, citations, and evidence cards.
