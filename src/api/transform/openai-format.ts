...
export function convertToOpenAiMessages(
	...
	for (const anthropicMessage of anthropicMessages) {
		if (typeof anthropicMessage.content === "string") {
			...
			// If a message also contains historical reasoning_details,
			...
			if (anthropicMessage.role === "user") {
				...
				toolMessages.forEach((toolMessage) => {
					...
					openAiMessages.push({
						...
						// Use "(empty)" placeholder for empty content to satisfy providers that require non-empty content
				...
				// from providers that require non-empty content. Images always have content (base64 data).
				...
				// Check if the message has reasoning_details (preserved from historical provider responses)
				const messageWithDetails = anthropicMessage as any

				// Build message with reasoning_details BEFORE tool_calls to preserve
				// the order expected by providers like Alpha. Property order matters
				// when sending messages back to some APIs.
				const baseMessage: OpenAI.Chat.ChatCompletionAssistantMessageParam & {
					reasoning_details?: any[]
				} = {
					role: "assistant",
					// Use empty string instead of undefined for providers that require non-empty content
