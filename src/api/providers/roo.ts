...
import { rooDefaultModelId, getApiProtocol, type ImageGenerationApiMethod } from "@alpha-code/types"
...
// Extend OpenAI's CompletionUsage to include Alpha specific fields
interface RooUsage extends OpenAI.CompletionUsage {
	cache_creation_input_tokens?: number
	cost?: number
}

// Add custom interface for Alpha params to support reasoning
...
export class RooHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	protected override createStream(
		...
		// Get Alpha-specific reasoning parameters
	...
	override async *createMessage(
		...
		try {
			...
			const headers: Record<string, string> = {
				"X-Alpha-App-Version": Package.version,
			}

			if (metadata?.taskId) {
				headers["X-Alpha-Task-ID"] = metadata.taskId
			}
	...
	/**
	 * Generate an image using Alpha Cloud's image generation API
