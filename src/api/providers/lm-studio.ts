...
import { type ModelInfo, openAiModelInfoSaneDefaults, LMSTUDIO_DEFAULT_TEMPERATURE } from "@alpha-code/types"
...
export class LmStudioHandler extends BaseProvider implements SingleCompletionHandler {
	...
	override async *createMessage(
		...
		try {
			...
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with Alpha's prompts.",
			)
		}
	}
	...
	override getModel(): { id: string; info: ModelInfo } {
		const models = getModelsFromCache({
			provider: "lmstudio",
			baseUrl: this.options.lmStudioBaseUrl || "http://localhost:1234",
		})
