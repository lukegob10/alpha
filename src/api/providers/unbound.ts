...
import { type ModelInfo, type ModelRecord, unboundDefaultModelId, unboundDefaultModelInfo } from "@alpha-code/types"
...
export class UnboundHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			defaultHeaders: {
				...DEFAULT_HEADERS,
				"X-Unbound-Metadata": JSON.stringify({ labels: [{ key: "app", value: "alpha-code" }] }),
			},
		})
	}
	...
	override async *createMessage(
		...
		const completionParams: UnboundChatCompletionParamsStreaming = {
			...
			unbound_metadata: { originApp: "alpha-code", taskId: metadata?.taskId, mode: metadata?.mode },
