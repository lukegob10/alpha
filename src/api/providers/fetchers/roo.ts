...
/**
 * Fetches available models from the Alpha Cloud provider
 *
 * @param baseUrl The base URL of the Alpha Cloud provider
 * @param apiKey The API key (session token) for the Alpha Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				console.error("Error fetching Alpha Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch Alpha Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching Alpha Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch Alpha Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch Alpha Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch Alpha Cloud models: No response from server. Check Alpha Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch Alpha Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
